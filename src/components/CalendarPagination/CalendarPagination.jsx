...
const CalendarPagination = () => {
    ...
    for (let i=1; i<= daysOfMonth; i +=1) {
       ...
       arrOfDays.push({date: itemDate, day: i, id: crypto.randomUUID(), volume: userNorm, formDate: formattedDate});
    };
    ...
    return (<div className={css.container}>
        ...
        <button className={css.btn} onClick={()=>decreaseDate(date)}>{'<'}</button>
        <p className={css.text}>{month}, {year}</p>
        <button className={css.btn} onClick={()=>increaseDate(date)}>{'>'}</button>
        <SvgIcon
        ...
        <div><CalendarItemsList cards={arrOfDays}/></div>
